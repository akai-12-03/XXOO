...
@Controller
public class ToQddRechargeController extends WebController {
	 ...
	 * 生成充值信息
	 */
	@RequestMapping("qddApi/toLoanRecharge")
	public String toLoanRecharge(ModelMap map, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (isNumber(Amount)) {
			if(StringUtils.isEmpty(Amount) || Double.valueOf(Amount) >= 5000000){
				...
				return "redirect:/myhome/charge.html";
			}
			if(user != null) {
				...
				if (user.getUserType() == 2) {
					if(Double.valueOf(Amount) <= 20){
						map.addAttribute("msg","单笔最低金额必须大于20元");
						return "redirect:/myhome/charge.html";
					}
				} else {
			...
			if(user != null) {
				...
				ReturnURL = Global.getValue("qdd_notifyUrl") + "/qddApi/toLoanRechargeReturn.html";
				NotifyURL = Global.getValue("qdd_notifyUrl") + "/qddApi/toLoanRechargeNotify.html";
				...
				//System.out.println("充值请求参数:" + dataStr);
	 ...
	 * @return
	 */
	@RequestMapping("qddApi/toLoanRechargeReturn")
	public String toLoanRechargeReturn(ModelMap map, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String FeePlatform=params.get("FeePlatform");
		Fee = params.get("Fee");
		RechargeType = params.get("RechargeType");
		...
		//System.out.println("充值页面返回签名信息verifySignature=:" + dataStr);
		System.out.println("充值页面返回码ResultCode=:" + ResultCode);
	 ...
	 * @return
	 */
	@RequestMapping("qddApi/toLoanRechargeNotify")
	...
	public void toLoanRechargeNotify(ModelMap map, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(verifySignature)
		{
			if(ResultCode.equals("88"))
			{
				...
				if (userRecharge!=null &&userRecharge.getStatus() == -1) {
					// 用户账户增加充值金额
					try{
				...
				userRecharge.setUpdatedAt(NewDateUtils.getNowTimeStr());
				userRecharge.setPayResult("充值失败");
